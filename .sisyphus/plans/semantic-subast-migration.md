...
- [x] **C7**: `src/workspace.c3` migration — NO-OP. All 18 `.type_text` reads are dual-storage **cache writers** (copying parser-produced text into `MemberDecl.type_text` for the workspace symbol cache, plus one `mem::free` cleanup at :268). MemberDecl is itself a text-based struct; it carries no AST handle. Workspace.c3 is NOT a renderer/consumer — it produces the cache that downstream consumers read. To enable downstream MemberDecl consumers (the C5d/e DEFER blockers) requires a separate architectural extension (MemberDecl gaining AST handle), which is out of scope for "consumer migration." Workspace must keep copying text until Wave D removes the field. Marked done with rationale.
- [x] **C8a**: `src/kotlin/contracts.c3` migration — NO-OP (text-consumer DEFER). Single reader at :388 propagates TYPE_CHECK_EXPR `n.type_text` into `ContractEffect.type_text` for downstream flow analysis. Field is consumed by `flow.c3:1031` via `types::parse_type_text` — text-consumer pattern (same blocker as C5e). Migration requires sub-AST handle on ContractEffect + TypeRef builder; both deferred to Wave D. Rationale documented; field stays text.
- [x] **C8b**: `src/kotlin/cfg.c3` migration — NO-OP. Zero `.type_text` readers (verified grep). No migration needed.
- [x] **C9**: `src/kotlin/flow.c3` migration — NO-OP (text-consumer DEFER). All 5 readers (:371 propagation, :661/662 condition type extraction + parse, :686 cond-narrow parse, :1031 effect parse) feed `types::parse_type_text` — text-consumer pattern (same blocker as C5e/C8a). Migration requires sub-AST handles on Fact/ContractEffect + TypeRef builder; deferred to Wave D. Field stays text; consumers stay text-based.
- [x] **C10a**: `src/kotlin/types.c3` inference-helpers migration — NO-OP (text-consumer DEFER batch). 43 readers in this file; ~30+ are `types::parse_type_text(...)` consumers (lines 810-878, 1326-1435, 1710, 3218-3522, 4538-4600), the remainder are text-key string-compares (`pt == tp_names[t]`), text-propagators into result structs, or MemberDecl cache reads. types.c3 IS the type-resolution backbone — its job is text→TypeRef conversion, so virtually every reader is a text-consumer. Migration requires the Wave D unblockers: (a) sub-AST→TypeRef builder to replace `parse_type_text`, AND (b) MemberDecl AST handle for cross-file lookups. Both out of scope for Wave C "consumer migration." Field stays text; consumers stay text-based. Marked done with rationale.
